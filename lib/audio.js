...
function createTempAudioArtifactFromBuffer(audioBuffer, fileExtension, source) {
...
function loadGeneratedAudioArtifact(filePath, source) {
  ...
  return buildAudioArtifact({
    ...
    fileType: AUDIO_FILE_TYPE_BY_EXTENSION[extension] || "opus",
...
async function synthesizeVoiceAudioWithNativeTts(config, logger, params, deps = {}) {
  ...
  if (!artifact) {
    logger?.info?.(`feishu-voice skip unsupported native TTS format=${synthesis.fileExtension || "unknown"}, fallback to script synthesis`);
    return null;
  }
...
async function sendVoiceReply(config, logger, params) {
  ...
  if (!audioArtifact) {
    audioArtifact = await synthesizeVoiceAudioWithNativeTts(config, logger, { text }, {
