...
const { createVoiceReplyRouter, isObservationOnlyRoute } = require("./voice-reply-route");
...
function registerVoiceReplyHooks(api, config, deps = {}) {
  ...
  api.on("inbound_claim", (event, ctx) => {
    const isNewInbound = router.handleInboundLifecycleEvent(event, ctx);
    if (isNewInbound === false) return;
    dispatcher.clearPendingForInbound(ctx, "new_inbound");
  });

  api.on("message_received", (event, ctx) => {
    const isNewInbound = router.handleInboundLifecycleEvent(event, ctx);
    if (isNewInbound === false) return;
    dispatcher.clearPendingForInbound(ctx, "message_received_inbound");
  });

  api.on("before_dispatch", (event, ctx) => {
    const isNewInbound = router.handleInboundLifecycleEvent(event, ctx);
    if (isNewInbound === false) return;
    dispatcher.clearPendingForInbound(ctx, "before_dispatch_inbound");
  });

  api.on("before_tool_call", (event, ctx) => {
    if (event?.toolName !== "tts") return;
    if (!config.voiceReplyEnabled || config.voiceReplyMode === "off") return;
    if (config.promptToolTtsForText === true) return;
    if (!isFeishuChannelContext(ctx)) return;

    const route = router.resolveSessionTarget(ctx);
    if (isObservationOnlyRoute(route)) return;
    if (Number(route?.lastVoiceInboundAt || 0) <= 0) return;

    api.logger?.info?.(
      `feishu-voice blocked tts tool in voice inbound session (run=${ctx?.runId || "none"}, session=${ctx?.sessionKey || "none"})`
    );
    return {
      block: true,
      blockReason: "Feishu voice inbound replies use plugin-managed async audio; tts tool is disabled for this turn."
    };
  });
  ...
  api.on("before_agent_start", (_event, ctx) => {
    if (!config.voiceReplyEnabled || config.voiceReplyMode === "off") return;
    if (!isFeishuChannelContext(ctx)) return;
    dispatcher.bindActiveRun(ctx);
  });

  // 新版 OpenClaw 可在最终回复落盘前直接给插件最终文本。
  api.on("before_agent_reply", (event, ctx) => {
    if (config.enableBeforeAgentReply !== true) return;
