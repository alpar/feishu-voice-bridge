...
function createVoiceReplyStore() {
  return {
    ...
    activeRunIdBySessionKey: new Map(),
...
function pruneStaleVoiceReplyState(store, now = Date.now(), ttlMs = VOICE_REPLY_STATE_TTL_MS) {
  ...
  pruneTimestampMap(store.activeRunIdBySessionKey, now, effectiveTtlMs, VOICE_REPLY_STATE_LIMITS.activeRunIdBySessionKey);
