...
function resolveRequestedReceiveIdType(input, target) {
  const requested = typeof input === "string" ? input.trim() : "";
  if (requested === "chat_id" || requested === "open_id" || requested === "user_id") {
    return requested;
  }
  return resolveReceiveIdType(target);
}
...
async function createAudioMessage(config, logger, params) {
  ...
  const receiveIdType = resolveRequestedReceiveIdType(params.receiveIdType, target);
