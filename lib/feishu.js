...
async function uploadAudioToFeishu(config, logger, params) {
  ...
  logger?.info?.(`feishu-voice upload ok (account=${normalizeAccountId(params.accountId)}, durationMs=${params.durationMs}, fileKey=${fileKey})`);
...
async function createAudioMessage(config, logger, params) {
  ...
  const payload = await feishuJsonRequest(`${baseUrl}/open-apis/im/v1/messages?receive_id_type=${encodeURIComponent(receiveIdType)}`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
  ...
  logger?.info?.(`feishu-voice send ok (mode=create, target=${target}, receiveIdType=${receiveIdType}, messageId=${messageId || "unknown"}, chatId=${chatId || "unknown"})`);
...
function isFeishuChannelContext(ctx) {
  return typeof ctx?.channelId === "string" && ctx.channelId.toLowerCase() === "feishu";
}
