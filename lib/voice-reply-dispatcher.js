...
function createVoiceReplyDispatcher(params) {
  ...
  function resolvePendingRunKey(ctx, route) {
    ...
    const routeResolutionSource = typeof route?.resolutionSource === "string" ? route.resolutionSource : "";
    // latest_route 只说明“最近给这个账号发过谁”，
    // 不代表当前事件真的属于上一轮 run，因此不能再借旧 reply/target 命中 pending。
    const allowWeakRouteReuse = routeResolutionSource !== "latest_route";
  ...
  function shouldTreatLatestRouteAsWeak(route) {
    return typeof route?.resolutionSource === "string" && route.resolutionSource === "latest_route";
  }
  ...
  function schedulePendingFlush(runKey, pending) {
    ...
    if (allowNoTextFallback) {
      ...
      scheduleRunVoiceReplyFlush(runKey, "no_text_fallback", fallbackDelayMs);
    }
  }
  ...
  function enqueueVoiceReply(reply, ctx, source) {
    ...
    if (source === "before_agent_reply" && sessionKey) {
      if (store.transcriptEchoSkippedBySessionKey.get(sessionKey)) {
        api.logger?.info?.("feishu-voice skip before_agent_reply capture: transcript echo session");
        return;
      }
      const skippedTranscriptEchoText = store.transcriptEchoTextBySessionKey.get(sessionKey);
    ...
    if (shouldTreatLatestRouteAsWeak(route)) {
      api.logger?.info?.(
  ...
  function markTextSending(ctx, replyText = "") {
    ...
    const allowWeakRouteAliases = !shouldTreatLatestRouteAsWeak(route);
    ...
    const candidates = [
      ...
      allowWeakRouteAliases && route?.target ? resolveCanonicalRunKey(store, `target:${normalizeAccountId(route.accountId)}:${normalizeFeishuTarget(route.target)}`) : "",
      allowWeakRouteAliases && route?.replyToMessageId ? resolveCanonicalRunKey(store, `reply:${normalizeFeishuMessageId(route.replyToMessageId)}`) : ""
    ].filter(Boolean);
  ...
  function markTranscriptEchoSkipped(ctx, source, text = "") {
    ...
    if (sessionKey) {
      ...
      if (normalizedText) {
        store.transcriptEchoTextBySessionKey.set(sessionKey, normalizedText);
      }
    }
    const candidates = [
      runKey ? resolveCanonicalRunKey(store, runKey) : "",
