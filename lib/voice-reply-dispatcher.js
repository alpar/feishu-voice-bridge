...
function createVoiceReplyDispatcher(params) {
  ...
  function resolvePendingRunKey(ctx, route) {
    ...
    const directAliases = aliases.filter((alias) => !alias.startsWith("target:"));
  ...
  function clearPendingForInbound(ctx, reason = "new_inbound") {
    ...
    for (const [runKey, pending] of store.pendingRunVoiceByKey.entries()) {
      ...
      api.logger?.info?.(`feishu-voice cleared stale pending reply (run=${runKey}, reason=${reason}, target=${target || "unknown"})`);
    }
