...
const AUTHORITATIVE_SOURCES = new Set(["assistant_message", "before_agent_reply", "agent_end_snapshot", "message_sent"]);
...
function extractAssistantTextFromAgentMessage(message) {
  ...
  for (let index = content.length - 1; index >= 0; index -= 1) {
    ...
    if (part.type !== "text") {
      if (trailingTextParts.length > 0) break;
      continue;
    }
    ...
    trailingTextParts.unshift(part.text);
  }

  return trailingTextParts.join("\n").trim();
}

function extractLatestAssistantTextFromMessages(messages) {
  const entries = Array.isArray(messages) ? messages : [];
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    const text = extractAssistantTextFromAgentMessage(entries[index]);
    if (text) return text;
  }
  return "";
}
