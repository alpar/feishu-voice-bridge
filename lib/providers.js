...
const { resolveNativeAudioTranscriber } = require("./runtime");
...
function isNativeTtsAvailable(config, runtime) {
  ...
  pluginRuntime.ensureNativeCapabilities?.();
  return !!pluginRuntime?.hasNativeTts;
}
...
async function transcribeAudioWithNativeRuntime(config, runtime, req, logger = null) {
  const transcribeAudioFile = resolveNativeAudioTranscriber(runtime?.coreRuntime);
  if (!runtime?.hasNativeStt || typeof transcribeAudioFile !== "function") {
    return null;
  }
...
function buildProvider(config, logger, runtime, deps = {}) {
  ...
  return {
    ...
    isConfigured: () => {
      pluginRuntime.ensureNativeCapabilities?.();
      return pluginRuntime.hasNativeTts || pluginRuntime.hasToolTts;
    },
