...
function buildProvider(config, logger, runtime) {
  ...
  return {
    ...
    synthesize: async (req) => {
      ...
      let artifact = await synthesizeVoiceAudioWithNativeTts(config, logger, { text }, {
        runtime: pluginRuntime
      });
      if (!artifact) {
        artifact = synthesizeVoiceAudio(config, logger, {
          text,
