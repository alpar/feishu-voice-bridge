...
- 当前版本：`2026.4.8`
- 本版重点：修复长文本语音摘要未命中 OpenClaw 原生 LLM 摘要能力、退回规则截断的问题。

## 2026.4.8 升级说明

- 长文本语音摘要现在会优先调用 OpenClaw 正式 `summarizeText()` 能力生成摘要。
- 旧 `_test.summarizeText` 只保留为兼容兜底，不再作为主判断依据。
- 如果你之前听到的长文本语音只是正文前半段，现在应当改为 LLM 提炼后的摘要。
- 升级后建议重启 OpenClaw Gateway，并发一条明显超过 `maxReplyChars` 的长回复确认语音内容。

## 2026.4.7 升级说明

- 插件已不再依赖旧版 `before_agent_start` 绑定当前 run。
- 如果你之前在 `openclaw doctor` 里看到“使用了旧版 before_agent_start 钩子”的提示，升级到这一版后应当消失。
- 这次升级不改自动语音回复的核心状态机，只做宿主兼容性迁移和回归测试补强。
